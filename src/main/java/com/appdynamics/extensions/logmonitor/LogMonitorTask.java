...
public class LogMonitorTask implements AMonitorTaskRunnable {
    ...
    private void publishEvents(LogMetrics logMetrics) {
        ...
        if(events.size() != 0) {
            ...
            // NEW CODE: Using ProxyEventsServiceConnector
            ProxyEventsServiceConnector proxyConnector = new ProxyEventsServiceConnector(monitorContextConfiguration);
            proxyConnector.publishEvents(SCHEMA_NAME, eventsToBePublished);
            // --- MODIFICATION END ---
        }
        else {
